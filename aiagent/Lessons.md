...
- **実装前の設計見直し**: セットアップフェーズ完了後、実装開始前は設計の見直しの良いタイミング。実際のユースケースを再検討し、ユーザビリティと技術的複雑さのバランスを取った設計に改良すること。
- **ユニークキー戦略の設計**: 単純な自動ハッシュと、ドメイン知識を活用したビジネスキーは明確に区別すべき。動画IDや日付など、ビジネスロジック上重要なユニークキーはユーザーが指定できる柔軟性を提供しつつ、デフォルトでは簡単な設定で利用できる段階的複雑性を採用すること。
