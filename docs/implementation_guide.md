...
| カテゴリ                         | 技術                                                       | 目的・理由                                                                                |
| :------------------------------- | :--------------------------------------------------------- | :---------------------------------------------------------------------------------------- |
| **パッケージ管理**               | npm                                                        | Node.jsの標準パッケージマネージャであり、広く利用されているため。                         |
| **言語**                         | [TypeScript](https://www.typescriptlang.org/)              | 静的型付けにより、開発時のエラーを早期に検出し、コードの堅牢性を向上させるため。          |
| **ブラウザ拡張フレームワーク**   | [Plasmo](https://www.plasmo.com/)                          | ブラウザ拡張機能開発の定型的な作業を自動化し、開発効率を向上させるため。                  |
...
Kansuは、Plasmoフレームワークが提供するファイルベースのルーティングと役割分担に基づき、以下の主要なコンポーネントで構成されます。
...
- **Options UI (`options.tsx`)**:
  - **拡張機能全体の設定**を管理するページです。主な責務は以下の通りです。
    1. **抽出ルールの管理**: データ抽出のルール（対象URL、CSSセレクタ等）をサービスごとに設定・管理します。
       - **主要コンポーネント**: `SettingsForm` (抽出ルール設定フォーム)。
...
```json
{
  ...
  "activateUrlPattern": "https://example.com/items/*",
